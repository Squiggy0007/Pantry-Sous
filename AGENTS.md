...
# Stocker — iOS App Project README

## Overview
**Stocker** is a personal iOS app for managing household food inventory and discovering recipes based on what you actually have. Built for a family managing grocery budgets, the app helps reduce food waste and simplify meal planning. The core philosophy: **no assumptions — if it's not in your ingredients list, it's missing.**
...
|---|---|---|---|
| 0 | Ingredients | cabinet | Manage inventory by category |
| 1 | Recipes | fork.knife | Recipes matched to your inventory |
| 2 | Browse | safari | Browse trending + category recipes |
| 3 | Favorites | heart.fill | Saved recipes (offline) |
| 4 | Shopping | cart.fill | Shopping list + finish shopping flow |
...
Stocker
  └── Stocker
        ...
        │     └── MockBrowseData.swift        (130 browse recipes + BrowseCategory enum)
        ├── Utilities
        │     ├── StringExtensions.swift      (titleCased, removingHTMLTags)
        │     └── IngredientNormalizer.swift  (150+ families, strict matching, pepper fix)
        ├── ViewModels
        │     ├── InventoryViewModel.swift
        │     ├── RecipesViewModel.swift
        │     └── BrowseViewModel.swift
        ...
        │     ├── EditIngredientView.swift    (grid category picker)
        │     ├── BarcodeScannerView.swift
        │     ├── RecipesView.swift           (Ready to Make, Almost There, score badges)
        │     ├── RecipeCardView.swift        (score badge)
        │     ├── RecipeDetailView.swift      (toast notification, smart quantities)
        │     ├── BrowseView.swift            (category pills + vertical feed)
        │     ├── FavoritesView.swift
        │     ├── SavedRecipeDetailView.swift
        │     ├── ShoppingListView.swift      (grouped by category, emoji headers)
        │     ├── AddShoppingItemView.swift   (auto-categorization)
